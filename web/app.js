...
function updateEpsilon(){const epsilon=Number(slider.value),halfWidth=3.04/Math.sqrt(epsilon);document.querySelector("#epsilonValue").textContent=epsilon.toFixed(2);document.querySelector("#ciLow").textContent=`+${(14.2-halfWidth).toFixed(1)}%`;document.querySelector("#ciHigh").textContent=`+${(14.2+halfWidth).toFixed(1)}%`;const width=Math.min(82,28+30/epsilon);document.querySelector("#ciBand").style.left=`${(100-width)/2}%`;document.querySelector("#ciBand").style.right=`${(100-width)/2}%`;document.querySelector("#privacyCopy").textContent=epsilon<1?"식별력보다 프라이버시를 우선하는 설정입니다. 신뢰구간이 넓어집니다.":"식별력이 높아지지만 릴리즈당 프라이버시 비용도 커집니다."}
